...
select first_name,lower(first_name)  as lower from customer;
...
select first_name,substr(first_name,2,1) from customer;
...
select avg(rental_rate) as mean from film;
select title, rental_rate, avg(rental_rate) as mean from film group by title,rental_rate having rental_rate > mean ;
select title, rental_rate, avg(rental_rate) as mean from film where rental_rate > avg(rental_rate) ;
...
select f.title,f.film_id from rental as r inner join inventory as i on r.inventory_id=i.inventory_id inner join film as f on i.film_id=f.film_id where return_date between '2005-05-29' and '2005-05-30';
select count(f.film_id) from rental as r inner join inventory as i on r.inventory_id=i.inventory_id inner join film as f on i.film_id=f.film_id where return_date between '2005-05-29' and '2005-05-30';
...
# indexes,commit and roll back,view definition and TCL